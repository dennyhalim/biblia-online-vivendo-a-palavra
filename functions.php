...
if(file_exists(plugin_dir_path(__FILE__) . '/bovp_xtras.php')) require_once plugin_dir_path(__FILE__) . 'bovp_f.php'; 
...
define('BOVP_VERSION_NAME', get_option('bovp_version_name'));
define('BOVP_TABLE_NAME', get_option('bovp_table_name'));
define('BOVP_PAGE', get_option('bovp_page'));
define('BOVP_STATUS', get_option('bovp_status'));
define('BOVP_RECORDS', get_option('bovp_records'));
define('BOVP_THEME', get_option('bovp_theme'));
define('BOVP_URL', get_option('home').'/index.php?page_id='.get_option('bovp_page'));

// GLOBALS

$bovp_versions_inf = get_option('bovp_versions');
...
function showStatus() {
	...
	$cmd = "SELECT * FROM `". BOVP_TABLE_NAME ."` RAND LIMIT 5";
...
function PluginShowBible($content) {
	...
	if (is_page(BOVP_PAGE)) {  
		...
		if (isset($vars['bk']) AND ($vars['bk']) !=0 ) { 
				...
				$book_name = PluginBookInfo($bk,'name');

				$lastpage = PluginBookInfo($bk,'pages');
				...
				if(empty($vars['sh'])) { 
					...
					$sql = "SELECT * FROM `" . BOVP_TABLE_NAME . "` WHERE `book`=1 AND `cp`=1"; 
		...
		if (isset($bovp_search) and ($bovp_search != "")) { // SEARCH RESULTS							
				...
				foreach ($bovp_results as $bovp_results_rows){	
					...
					$text = $bovp_results_rows->text;				
						...
						$text = preg_replace_callback("/$bovp_search/i", "bovpCallback", $text);
						...
						$book_name = PluginBookInfo($book, 'name'); 

						$qtdCapitulos = PluginBookInfo($book, 'pages');									
		...
		$bovp_version =  __('Version: ', 'bovp') . BOVP_VERSION_NAME;	
...
function PluginSoftInstall(){ 
	...
	if($install_type==1) { // Soft update only
		...
		if(PluginTableExist(BOVP_TABLE_NAME)) {$wpdb->query("DROP TABLE `" . BOVP_TABLE_NAME . "`");}
		...
		add_option("bovp_version_name", '', '', 'yes');
		add_option("bovp_table_name", '', '', 'yes');
		add_option("bovp_versions", PluginBiblesInstalls(), '', 'yes');
		add_option("bovp_status", false, '', 'yes');
		add_option("bovp_records", '', '', 'yes');
...
function PluginInsertData($bovp_install){
	...
	if($bovp_create_table) {
		...
		if((int)$bovp_row_count == (int)$bovp_records) { 
			...
			update_option("bovp_records", bovp_row_count);
			update_option("bovp_bd_version", '1.5.1');
			update_option("bovp_version", $bovp_install);
			update_option("bovp_version_name", $bovp_version_name);
			update_option("bovp_table_name", $bovp_wp_table_name);
		...
		} else {return array(false,'RC ' . (int)$bovp_row_count .' <> BR ' . (int)$bovp_records);}
...
function bovp_show_verse($return_type = 'show', $size = '0'){
	...
	$daily_verse = $wpdb->get_row($sql);
		...
		if($daily_verse){
			...
			$bovp_daily_verse = array();
				...
				$bovp_daily_verse['book_name'] = PluginBookInfo($daily_verse->book,'name');
...
function PluginBookInfo($book,$item) {
	...
	if( empty($bovp_array_books) OR !isset($bovp_array_books)){
		...
		PluginBookArrayConstruct(BOVP_TABLE_NAME);
		$bovp_array_books =  get_option("bovp_array_books"); 
...
<form method=\"post\" action=\"" . BOVP_URL . "\" name=\"bovp_form_search\" class=\"bovp_form_search bovp_clear\" >
...
*/
	...
	#Default values
	var $open_group = '<div class="bovp_clear bovp_group">'; 
	var $close_group = '</div>';
	...
	function prepareGroup($group){
		...
		switch ($group) {
			...
			case 'header':
				...
				$return = "<header class='bovp_header bovp_clear'>\n";
			...
			case 'content':
				...
				$return = "<article class='bovp_text bovp_clear'>\n";
			...
			case 'pagination':
				...
				$return = "<nav class='bovp_pagination bovp_clear'>\n";
			...
			case 'version':
				...
				$return = "<section class='bovp_version bovp_clear'>\n";
			...
			case 'footer':
				...
				$return = "<footer class='bovp_footer bovp_clear'>\n";
			...
			case 'logo':
				...
				$return = "<section class='bovp_logo bovp_clear'>\n";
	...
	function showBible($args, $t = 0){
		...
		if(isset($_REQUEST['stats']) && $_REQUEST['stats']==true) {
...
class bovp_widget_search extends WP_Widget {
    ...
    function widget($args, $instance){
		...
		echo "<div class=\"bovp_search_widget bovp_clear\">";
		echo "<form method=\"post\" action=\"" . BOVP_URL . "\" name=\"bovp_form_search\" class=\"bovp_form_search bovp_clear\">";
...
class bovp_widget_index extends WP_Widget {
  ...
  /* Displays the Widget in the front-end */
    function widget($args, $instance){
		...
		echo "<form method=\"get\" action=\"" . BOVP_URL . "\" name=\"bovp_form_index\" class=\"bovp_form_search bovp_clear\">";
...
function pluginShowForm() {

	echo "<div class=\"bovp_search_extra bovp_clear\">";
	echo "<form method=\"post\" action=\"" . BOVP_URL . "\" name=\"bovp_form_search\" class=\"bovp_form_search bovp_clear\">";
	echo "<input name=\"page_id\" type=\"hidden\" value=\"" . BOVP_PAGE . "\"/>";
	echo "<input class=\"bovp_seach_input\" type=\"text\" id=\"sh\" name=\"sh\" placeholder=\"" . __('Search','bovp') . "\" >";
	echo "<select name=\"bk\" id=\"bovp_bk\" class=\"bovp_bk\">" . book_select("var") . "</select>";
	echo "<select type=\"text\" name=\"cp\" id=\"bovp_cp\" class=\"bovp_cp\"></select></div>";
	echo "<button class=\"bovp_button\" type=\"submit\">" . __('Send','bovp') . "</button>";
	echo "</form>";
	echo "</div>";
