...
function bovpShowBible($content) {
	...
	if (is_page(BOVP_PAGE) AND defined('BOVP_VERSION') AND  BOVP_VERSION != -1) {  
		...
		if(isset($_REQUEST['test_plugin'])){showStatus(); return false;}
...
function bovpSetVerse() {

	global $bovp_daily_verse;
...
function PluginShowMessage($message, $errormsg = false) {
	...
	/* ADD in the 1.5.1 Version */
	function bovpPrettyUrlsActivate(){
		...
		if($tags){
			...
			foreach ($tags as $tag) {
				...
				$name_tag = $tag['name'];
					...
					$tag_link = trim($name_tag);

					$link_url = get_option('siteurl') . "?tag=" . $tag_link;

					$link = '<a class="bovp_tag_link" href="' . $link_url . '">' . $tag_link . '</a>';
											
					$bovp_replace = str_replace($tag_link,$link,$bovp_replace);
			}
	...
	<form method="post" action="options.php">
	      ...
	      <?php settings_fields( 'bovp_options' ); ?>
	        ...
	        <fieldset class="bovp_fieldset">
	          ...
	          <?php bovpListWordpressPages ('bovp_page', PluginGetAllPages ()); ?>
	        </fieldset>
...
<?php
	...
	function bovpInit() {
		...
		if(!isset($bovp_vars['sh']) OR empty($bovp_vars['sh'])) $bovp_vars['sh'] = (int) 0; else {$bovp_vars['sh'] = urldecode($bovp_vars['sh']);};
...
// filters
add_filter('query_vars','bovpIncludeVars'); //include query vars for bovp.
...
if(BOVP_FURL_STATS) { bovpPrettyUrlsActivate(); }
